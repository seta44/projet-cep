	...
	*/
	...
	public function setError($status, $label, $code)
	{
		...
		if (!isset($display_errors))
			$display_errors = strtolower(ini_get('display_errors')) != 'on';
		if (isset($this->objOutput))
			$this->objOutput->setStatus($status);
		$this->errors[] = $display_errors ? array($code, $label) : 'Internal error. To see this error please display the PHP errors.'; /* FSA 505 */
	}
